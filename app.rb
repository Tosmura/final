...
get "/restaurants/:id" do
    ...
    @rsvps = rsvps_table.where(:restaurant_id => params["id"]).to_a
    # SELECT COUNT(*) FROM rsvps WHERE event_id=:id AND going=1
    @count = rsvps_table.where(:restaurant_id => params["id"], :going => true).count
    view "restaurant"
end

# Form to create a new RSVP
get "/restaurants/:id/rsvps/new" do
    @restaurant = restaurants_table.where(:id => params["id"]).to_a[0]
    view "new_rsvp"
end
...
post "/restaurants/:id/rsvps/create" do
    rsvps_table.insert(:restaurant_id => params["id"],
                       :going => params["going"],
